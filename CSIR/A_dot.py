...
def animate_path_on_grid(win, path_positions, grid, delay=50, offset_x=WIDTH):
    for row, col in path_positions:
        pygame.time.delay(delay)
        spot = grid[row][col]
        pygame.draw.circle(win, BLUE, 
            (spot.x + spot.width // 2 + offset_x, spot.y + spot.width // 2), 
            spot.width // 3)
...
def algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        if current == end:
            ...
            start.make_start()
            return path  # ✅ Return path positions here
...
def silent_algorithm(grid, start, end):
    ...
    while not open_set.empty():
        ...
        if current == end:
            reconstruct_path(came_from, end, lambda: None)
            end.make_end()
            return True
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]:  # LEFT
                ...
                if not start1:
                    start1 = spot
                    start1.make_start()
                elif not end1 and spot != start1:
                    end1 = spot
                    end1.make_end()
                elif spot != end1 and spot != start1:
                    spot.make_barrier()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start1 and end1:
                    ...
                    path_positions = algorithm(lambda: draw_both(win, grid1, grid2, ROWS, width), grid1, start1, end1)
                    if path_positions:
